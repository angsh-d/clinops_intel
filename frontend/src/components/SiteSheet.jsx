...
import { X, TrendingUp, TrendingDown, AlertCircle, MessageCircle } from 'lucide-react'
      ...
      >
        <div className="sticky top-0 bg-apple-surface/90 backdrop-blur-xl border-b border-apple-border px-6 py-4 flex items-center justify-between z-10">
          <div>
            ...
            <p className="text-caption text-apple-secondary">{selectedSite.country}</p>
          </div>
          <button
            onClick={() => setSelectedSite(null)}
        ...
        <div className="p-6 space-y-6">
          ...
          <AISummary
            summary="Entry lag elevated following CRA transition. Query backlog concentrated on Lab Results pages. Enrollment tracking to target."
            onAsk={() => handleInvestigate(`Why is ${selectedSite.id} showing elevated metrics?`)}
          />
          ...
          <DomainCard
            ...
            status="warning"
            summary="Entry lag spike during CRA transition, now recovering"
            metrics={[
              { label: 'Entry Lag', value: '4.2d', trend: 'down', note: 'was 16.2d peak' },
              { label: 'Open Queries', value: '23', trend: 'stable' },
              { label: 'Query Rate', value: '0.95', note: 'avg: 0.73' }
            ]}
          ...
          <DomainCard
            ...
            status="healthy"
            summary="Enrollment decelerated during CRA transition, recovering"
            metrics={[
              { label: 'Screened', value: '8', trend: 'stable' },
              { label: 'Randomized', value: '3', note: 'Target: 5' },
              { label: 'Screen Rate', value: '0.8/mo', note: 'was 1.2' }
            ]}
            onInvestigate={() => handleInvestigate(`Why did enrollment slow at ${selectedSite.id}?`)}
          />
          
          <AlertsSection alerts={[
            { severity: 'warning', message: 'Query backlog above threshold', time: '2h ago' },
            { severity: 'info', message: 'CRA transition detected', time: '2 weeks ago' }
          ]} />
...
function StatusBadge({ site }) {
  ...
  const { bg, text, label } = config[site.status] || config.healthy
  ...
  return (
    <div className="relative pl-4 border-l-2 border-gradient-to-b from-[#5856D6] to-[#AF52DE]">
      ...
      <div className="flex items-center gap-2 mt-3">
        <span className="text-caption text-apple-secondary">Conductor Synthesis · 92% confidence</span>
        <button
          onClick={onAsk}
    ...
    <div className={`card p-5 border-l-4 ${statusColors[status]}`}>
      ...
      <div className="mt-4 pt-3 border-t border-apple-border">
        <p className="text-caption text-apple-secondary">
          Source: EDC extract · Updated 2h ago · Confidence: 92%
        </p>
      </div>
    </div>
  )
